...
const OcorrenciaForm = ({ existingOcorrencia, onSuccess }) => {
    ...
    // Efeito para preencher o formulário ao editar uma ocorrência
    useEffect(() => {
    ...
    const handleArmaChange = (index, e) => {
        ...
        const upperValue = (name === 'modelo' || name === 'marca' || name === 'calibre' || name === 'numero_serie') ? value.toUpperCase() : value;
    ...
    const handleArmaSuggestionClick = (suggestion) => {
        ...
        novasArmas[activeSuggestionIndex] = { ...armaAtual, modelo_catalogado: suggestion.id, modelo: suggestion.modelo, tipo: suggestion.tipo, marca: suggestion.marca, calibre: suggestion.calibre };
        setOcorrencia(prev => ({ ...prev, armas_apreendidas: novasArmas }));
        setArmaSuggestions([]);
